...
export default function Cart({props}) {
    const {cart, setCart,handleRemove,handleDeleteAll, increaseCartNumber,reduceCartNumber, total, totalPrice }  = useCart();
                ...
                {cartItem?.map((row, index) => (
                  <div className="bg-white flex ml-80 mr-80 h-60 items-start">
                    ...
                    <div className="ml-8 flex mt-16">
                      <div
                        className="border border-slate-500 w-8 h-8"
                        onClick={() => reduceNumber(index)}
                      >
                        <div className="pl-2.5 pt-0.5">-</div>
                      </div>

                      <div className="border border-slate-500 w-16 h-8">
                        <div className="mt-1 ml-6">{row?.quantity}</div>
                      </div>
                      <div
                        className="border border-slate-500 w-8 h-8"
